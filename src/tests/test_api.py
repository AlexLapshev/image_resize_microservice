...
async def test_get_transaction_status(app, image_id, status):
...
async def test_get_resized(app, image_id, status):
...
async def test_send_image_data(app, test_data, status):
...
async def test_send_image_no_field(app, test_data, status):
    data = FormData()
    try:
