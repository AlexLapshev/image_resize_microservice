...
async def send_picture(request: Request) -> json_response:
    if int(request.headers['Content-Length']) > 11**7:
        return json_response({"error": "image size is too big"})
    ...
    return json_response({"error": "incorrect request"}, status=400)


@routes.get("/api/v1/get_transaction_status/")
async def get_status(request: Request) -> json_response:
